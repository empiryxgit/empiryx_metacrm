...
**Confirmed dependency (via live testing, not just Meta's docs):** reading a Page's lead forms/leads that were created through Ads Manager — i.e. virtually every real Meta Lead Ads campaign — fails with `(#200) Requires pages_manage_ads permission to manage the object` unless `pages_manage_ads` is also granted, even with `leads_retrieval` + `pages_show_list` + `pages_read_engagement` all present. RUTA's OAuth scope list and App Review submission now include `pages_manage_ads` for exactly this reason — see its own section below.

---

## pages_manage_ads

RUTA is a multi-tenant CRM: each business connects its own Meta account via its own OAuth login. Meta creates Lead Ads forms through Ads Manager, so reading a Page's lead forms/submissions (via `leads_retrieval`) requires `pages_manage_ads` even for read-only access — confirmed via a real Graph API Explorer call, not just Meta's documented note. RUTA uses this strictly read-only, exactly like `ads_read`: to list a tenant's own lead forms and fetch their own submitted leads into their CRM pipeline. RUTA never creates, edits, or manages ads with this permission.
