...
// Vercel Function for the same reason agency invites/onboarding are -
// Vercel Hobby's 12-Function cap, already fully used (see this file's own
// header comment) - so vercel.json routes /api/agency/* here with
// ?resource=agency (plus ?action=). Falls through to the pre-existing
// behavior above whenever ?resource= is absent.
//
...
// granular permission for "runs an agency" the way there is for most
// modules, since every agency account today is its own Owner (see
...
// (handleCollection) - no extra permission gate beyond
// handleAgencyResource's own accountType check, same as every other
// agency action.
